...
int main(void) {
  size_t lbuf_inc = 5; // Increment the size this much if we cant fit an entire line in
  for (;;) {
    size_t lbuf_size = 5;
    char* lbuf = malloc(lbuf_size);
    for (;;) {
      /* In a loop, we're going to keep calling fgetline and reading in `lbuf_inc` number of
	 characters into the in lbuf until we've accomodated the whole line, increasing the
	 size of lbuf each time only a partial line can be read (provided no errors
	 A key thing to remember is that fgetline returns null terminated strings.
	 Thats why we have to use the ternary operator when we copy: if we don't overwrite
	 the "last" position of `lbuf`, then we'll have a \000 there and fprintnumbers cant
	 do its job.
      */
      if(fgetline(lbuf_inc, 
		  &lbuf[lbuf_size == lbuf_inc ? 0 : lbuf_size - lbuf_inc - 1],
		  stdin)) {
	// We've read the whole line successfully
	size_t n;
	    ...
	    free(nums);
	    free(lbuf);
	    break; // We've read one full line, on to the next
      } else {
	    if (lbuf[0]) {  /* a partial line has been read */
	      if (feof(stdin)) {
		return EXIT_FAILURE;
	      }
	      else { // we're gonna need to read more lines, guys
		lbuf_size += lbuf_inc - 1;  // "-1" to account for the null termination that we're skipping
		lbuf = realloc(lbuf, lbuf_size);
		if (!lbuf) {
		  printf("Could not reallocate lbuf. Exiting!\n");
		  return EXIT_FAILURE;
		}
	      }
	    }
